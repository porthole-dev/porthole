...
scp "${TK_SSH_OPTS[@]}" tools/ph-greetd-login.py tools/ph-touch.py tools/ph-key.py tools/ph-webeval.py tools/ph-webvq.py "$PHONE:/tmp/" >/dev/null 2>&1
printf '%s' "$TK_LOGIN_PASSWORD" | ssh "${TK_SSH_OPTS[@]}" "$PHONE" \
  'sudo -n env TK_LOGIN_PASSWORD="$(cat)" python3 /tmp/ph-greetd-login.py '"$PORTHOLE_USER" 2>&1 | tail -1
sleep 15
...
scp "${TK_SSH_OPTS[@]}" "$HERE/sess.sh" tools/ph-rangehttp.py "$PHONE:/tmp/" >/dev/null 2>&1
...
tk_run ". /tmp/sess.sh; pkill -x epiphany; sleep 2; rm -f ~/.local/share/epiphany/session_state.xml; \
  (cd ~ && setsid python3 /tmp/ph-rangehttp.py 8080 ~ >/tmp/http.log 2>&1 &); sleep 2; \
