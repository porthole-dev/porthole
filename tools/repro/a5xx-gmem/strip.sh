...
TAG=${1:-base}
N=${2:-40}
rm -rf /home/user/strip-$TAG; mkdir -p /home/user/strip-$TAG
(cd /home/user && setsid python3 /tmp/tk-rangehttp.py 8080 /home/user >/tmp/http.log 2>&1 &) 2>/dev/null
sleep 2
...
i=0
while [ $i -lt $N ]; do
  timeout 40 grim /home/user/strip-$TAG/s$(printf %03d $i).png 2>/dev/null
  i=$((i+1)); sleep 3
done
echo "[$TAG] captures=$(ls /home/user/strip-$TAG/*.png 2>/dev/null | wc -l) faults=+$(( $(sudo -n dmesg | grep -c 'gpu fault') - F0 ))"
du -sh /home/user/strip-$TAG
