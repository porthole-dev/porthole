...
# scope: generic
# needs: on-device as the session user (tk-webeval.py, tk-webvq.py, tk-touch.py,
#        tk-gesture-bench.py in /tmp; /tmp/sess.sh exporting the session env;
...
# tk-webarm.sh LABEL [ENV...] -- one YouTube arm with the page state PROVEN
...
#
#   tk-webarm.sh base
#   tk-webarm.sh sysmem "FD_MESA_DEBUG=sysmem"
L=$1; X=${2:-}
...
sleep 8
t1=$(python3 /tmp/tk-webeval.py 'document.querySelector("video").currentTime' 2>/dev/null); sleep 3
t2=$(python3 /tmp/tk-webeval.py 'document.querySelector("video").currentTime' 2>/dev/null)
adv=$(python3 -c "import sys; print('advancing' if float('${t2:-0}')-float('${t1:-0}')>2 else 'stuck')")
if [ "$adv" != "advancing" ]; then sudo -n python3 /tmp/tk-touch.py tap 720 600; sleep 6; t1=$(python3 /tmp/tk-webeval.py 'document.querySelector("video").currentTime'); sleep 3; t2=$(python3 /tmp/tk-webeval.py 'document.querySelector("video").currentTime'); adv=$(python3 -c "print('advancing' if float('${t2:-0}')-float('${t1:-0}')>2 else 'stuck')"); fi
echo "[$L] $(python3 /tmp/tk-webeval.py 'var v=document.querySelector("video"); JSON.stringify({w:v.videoWidth,h:v.videoHeight,paused:v.paused,t:Math.round(v.currentTime),rs:v.readyState})') $adv"
[ "$adv" = "advancing" ] || { echo "[$L] VIDEO NOT PLAYING -- arm void"; exit 1; }
S0=$(wc -l < /tmp/wl.log); sleep 10; S1=$(wc -l < /tmp/wl.log)
python3 /tmp/tk-webvq.py 2 1 | tail -1; python3 /tmp/threadcpu.py | head -1
sed -n "${S0},${S1}p" /tmp/wl.log > /tmp/wl-$L-idle.log
S0=$(wc -l < /tmp/wl.log); sudo -n python3 /tmp/tk-gesture-bench.py drag 720 2400 720 900 500 4 >/dev/null 2>&1; S1=$(wc -l < /tmp/wl.log)
