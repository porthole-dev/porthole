...
def main():
    ...
    if sha(packed) == sha(built):
        ...
        print("      install packed an apk other than the one just built. Two causes,")
        print("      and they need different fixes:")
        print("      1. the stale-APKINDEX bug -- the apk exists but is not indexed, so")
        print("         install picked an older one. Re-run the build so that")
        print("         `pmbootstrap index` runs CHAINED to `build --envkernel` (the")
        print("         chroot must still be mounted; a standalone index after shutdown")
        print("         fails with 'No private key found').")
        print("      2. a CACHED apk of an older pkgrel that install prefers outright.")
        print("         Re-running does NOT clear this -- on 2026-09-20 two identical")
        print("         re-runs reproduced it byte for byte, because the envkernel rung")
        print("         packages a dev apk (…_p<date>-r0), moves it to .stale-devpkgs,")
